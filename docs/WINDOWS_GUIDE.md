...
> ⚠️ **换电脑 / 移动目录后必须重建快捷方式**：直接复制 `.lnk` 不能用。请双击根目录 `repair-shortcut.cmd`，脚本会按**当前电脑**重建目标程序、启动参数与图标；只改图标路径会出现「图标有了但双击打不开」。若弹窗提示找不到 Python 3.12+，需先安装 Python 并保证 `py` 或 `python` 在 PATH 中。
