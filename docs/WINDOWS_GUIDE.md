...
> ⚠️ **换电脑 / 移动目录后必须重建快捷方式**：直接复制 `.lnk` 不能用。请双击根目录 `repair-shortcut.cmd`，脚本会按**当前电脑**重建目标程序、启动参数与图标。**设计说明**：快捷方式直接采用 Python 原生 GUI 进程（`pythonw.exe`）运行 `server.py --launcher`，彻底规避任何 PowerShell 中转与 Base64 编码，从根源消除杀毒软件误报（如 `TrojanDownloader/LNK.Agent.g`）并实现真·零黑框瞬时启动。若弹窗提示找不到 Python 3.12+，需先安装 Python 并保证 `py` 或 `python` 在 PATH 中。
